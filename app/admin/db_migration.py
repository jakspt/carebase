...
class DataMigrator(SQLBase, MongoBase):
        ...
        for patient in patients:
            ...
            for appt in appointments:
                ...
                for treatment in treatments:
                    ...
                    medikamente_list = [
                        {"pzn": med[0], "name": med[1]} for med in medications
                    ]
                ...
                appointments_list.append(
                    {
                        ...
                        "uhrzeit": time_str,
                        "grund": appt[3],
                        "arzt": {"svnr": appt[4], "name": appt[5]},
                        "behandlungen": behandlungen_list,
                    }
                )

                # Necessary for the "computed pattern" used for the doctor's usecase (usecase 1)
                if behandlungen_list:
                    self._update_doctor_revenue(
                        doctor_svnr, appt_date.year, behandlungen_list
                    )
    ...
    def _update_doctor_revenue(
        self, doctor_svnr: str, year: int, treatments: list[dict]
    ):
        """
        Helper: Sums up costs from a list of treatments and updates the doctor's yearly stats.
        """
        total_revenue = sum(t["kosten"] for t in treatments)
        ...
        if total_revenue > 0:
            ...
            doctor_collection.update_one(
                {"_id": doctor_svnr}, {"$inc": {cost_field: total_revenue}}
            )
    ...
    def migrate_to_doctor_collection(self):
        ...
        for row in rows:
            doctor_doc = {
                ...
                "abteilung": {"name": row[6], "gebäude": row[7], "stockwerk": row[8]},
            }
